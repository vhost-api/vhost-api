...
class User
  ...
  property :quota_vhost_storage, Integer, required: true, min: 0, max: (2**64 - 1), default: 104857600 # unsigned bigint, 100MiB default
  ...
  property :quota_dns_zone_records, Integer, required: true, min: 0, default: 10
  ...
  property :quota_mail_storage, Integer, required: true, min: 0, max: (2**64 - 1), default: 104857600   # unsigned bigint, 100MiB default
  ...
  def owner_of?(element)
    element.owner == self.id
  end

  def admin?
    Group.get(self.group_id).name == 'admin'
  end
end
